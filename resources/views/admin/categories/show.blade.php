...
<header class="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
    <div>
        <div class="flex items-center gap-3 mb-2">
            <a href="{{ route('admin.categories.index') }}" class="p-2 bg-card border border-border text-text/40 hover:text-primary rounded-xl transition-all">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            </a>
            <h1 class="text-3xl font-bold tracking-tight text-text capitalize">{{ $category->name }}</h1>
        </div>
        <p class="text-text/60 max-w-2xl">{{ $category->description ?? 'No extra details provided for this collection.' }}</p>
    </div>
    <div class="flex items-center gap-3">
        <a href="{{ route('admin.categories.edit', $category) }}" class="px-5 py-2.5 bg-card border border-border text-text font-bold rounded-2xl hover:border-primary/30 hover:bg-primary/5 transition-all">
            Edit Category
        </a>
        <a href="{{ route('admin.products.create', ['category' => $category->id]) }}" class="px-6 py-3 bg-primary text-white font-semibold rounded-2xl shadow-lg shadow-primary/20 hover:bg-primary-hover transition-all flex items-center gap-2">
    ...
    <table class="w-full text-left">
        <thead>
            <tr class="bg-bg/50 text-text/50 text-xs uppercase tracking-widest border-b border-border">
                <th class="px-8 py-4 font-semibold text-[10px]">Product</th>
            ...
            <tr>
                <td colspan="4" class="px-8 py-16 text-center">
                    <div class="flex flex-col items-center justify-center opacity-40">
                         <svg class="w-16 h-16 mb-4 text-text/20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>
                         <p class="font-black text-xl text-text/60 uppercase tracking-widest">No Products In This Category</p>
                         <p class="text-sm mt-1">This collection is currently waiting for items.</p>
                         <a href="{{ route('admin.products.create', ['category' => $category->id]) }}" class="mt-6 text-primary font-bold hover:underline">Add First Product →</a>
                    </div>
                </td>
            </tr>
            @endforelse
...
<!-- Image Viewer Modal (Same as Products Index) -->
