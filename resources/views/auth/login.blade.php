...
<div class="w-full max-w-[450px] p-10 bg-white/5 border border-white/10 backdrop-blur-2xl rounded-3xl shadow-2xl">
    ...
    @if ($errors->any())
        <div class="mb-6 p-4 bg-red-500/10 border-l-4 border-red-500 text-red-200 text-sm rounded-lg">
            {{ $errors->first() }}
        </div>
    @endif
    ...
    <form action="{{ route('login') }}" method="POST" class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <label class="flex items-center gap-2 text-sm text-slate-400 cursor-pointer group">
                <input type="checkbox" name="remember" class="w-4 h-4 rounded border-white/10 bg-white/5 accent-primary">
                <span class="group-hover:text-slate-200 transition-colors">Remember me</span>
            </label>
